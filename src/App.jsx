...
function ClinicalRelationPicker({ options, selected, onSelect, onClose }) {
  const [query, setQuery] = useState("");
  const normalized = query.trim().toLocaleLowerCase("es");
  const filtered = options.filter((item) => !normalized || `${item.type} ${item.date} ${item.title} ${item.detail}`.toLocaleLowerCase("es").includes(normalized));
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-900/55 p-4"><div className="flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><div><p className="text-xs font-bold uppercase text-hospital-600">Informe de práctica</p><h3 className="text-xl font-bold">Buscar atención relacionada</h3></div><button type="button" onClick={onClose} className="p-2 text-slate-400 hover:text-slate-700"><X/></button></div><div className="border-b p-4"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} className="control pl-10" placeholder="Buscar por fecha, profesional, Cabo, práctica, motivo..."/></div><p className="mt-2 text-xs text-slate-500">{filtered.length} de {options.length} registros</p></div><div className="min-h-0 flex-1 overflow-y-auto p-3"><button type="button" onClick={() => onSelect("")} className={`mb-2 w-full rounded-xl border p-4 text-left ${!selected ? "border-hospital-400 bg-hospital-50" : "border-slate-200 hover:bg-slate-50"}`}><p className="font-semibold text-slate-700">Sin vincular a una atención</p></button>{filtered.map((item) => <button key={item.value} type="button" onClick={() => onSelect(item.value)} className={`mb-2 w-full rounded-xl border p-4 text-left transition ${selected === item.value ? "border-hospital-400 bg-hospital-50 ring-1 ring-hospital-200" : "border-slate-200 hover:border-hospital-200 hover:bg-slate-50"}`}><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-bold text-cyan-700">{item.type}</span><span className="text-xs font-semibold text-slate-500">{item.date}</span></div><p className="mt-2 font-semibold text-slate-800">{item.title}</p><p className="mt-1 text-sm text-slate-500">{item.detail}</p></button>)}{!filtered.length && <div className="p-12 text-center text-slate-400"><Search className="mx-auto mb-3" size={32}/><p>No encontramos atenciones con ese criterio.</p></div>}</div><div className="flex justify-end border-t p-4"><button type="button" onClick={onClose} className="secondary">Cerrar</button></div></div></div>;
}
...
function ClinicalHistory({ patients, initialPatient, professionals, canCreate, onNotice }) {
  ...
  const emptyReport = () => ({ tipoPractica: "Laboratorio", fechaPractica: new Date().toISOString().slice(0, 10), titulo: "", descripcion: "", relacion: "", archivos: [] });
  const [record, setRecord] = useState(emptyRecord);
  const [report, setReport] = useState(emptyReport);
  const relationOptions = useMemo(() => [
    ...(history?.records || []).map((item) => ({ value: `record:${item.id}`, type: "Evolución clínica", date: String(item.fechaAtencion).slice(0, 16).replace("T", " "), title: item.descripcion, detail: item.profesional || "Sin profesional" })),
    ...(history?.cabos || []).map((item) => ({ value: `cabo:${item.id}`, type: "Cabo / práctica", date: formatDate(item.fecha), title: `Cabo N° ${item.numero} · ${item.prestaciones || item.tipoAtencion}`, detail: [item.profesionales, item.diagnosticos].filter(Boolean).join(" · ") || item.tipoAtencion })),
    ...(history?.appointments || []).map((item) => ({ value: `appointment:${item.id}`, type: "Turno", date: `${formatDate(item.fecha)} ${item.hora || ""}`.trim(), title: item.motivo || `Turno ${item.estado}`, detail: `${item.profesional || "Sin profesional"} · ${item.estado}` })),
  ], [history]);
  const selectedRelation = relationOptions.find((item) => item.value === report.relacion);
  ...
  const submitReport = async (event) => {
    event.preventDefault();
    if (!report.titulo.trim() || !report.tipoPractica.trim() || !report.fechaPractica) return onNotice("Completá práctica, fecha y título del informe.");
    if (!report.archivos.length) return onNotice("Adjuntá al menos un archivo PDF o imagen.");
    setSavingReport(true);
    try {
      const [relationType, relationId] = report.relacion.split(":");
      await savePracticeReport({
        ...report,
        pacienteId: Number(patient.codigo),
        registroId: relationType === "record" ? relationId : "",
        turnoId: relationType === "appointment" ? relationId : "",
        caboId: relationType === "cabo" ? relationId : "",
      });
      setCreatingReport(false);
      setReport(emptyReport());
      await load(patient);
      setActiveTab("reports");
      onNotice("Informe de práctica agregado a la historia clínica.");
    } catch (error) { onNotice(`No se pudo guardar el informe: ${error.message}`); }
    finally { setSavingReport(false); }
  };
  const openReportFile = async (file) => {
    const preview = window.open("", "_blank");
    try {
      const blob = await downloadPracticeReportFile(file.id);
      const url = URL.createObjectURL(blob);
      if (preview) preview.location.href = url;
      else onNotice("El navegador bloqueó la vista previa. Habilitá las ventanas emergentes para abrir el archivo.");
      window.setTimeout(() => URL.revokeObjectURL(url), 60000);
    } catch (error) { preview?.close(); onNotice(`No se pudo abrir el archivo: ${error.message}`); }
  };
  ...
  return <section className="space-y-5">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-2xl font-bold text-slate-800">Historia Clínica</h2><p className="mt-1 text-slate-500">{patientData.apellido}, {patientData.nombre} · DNI {patientData.dni || "—"}</p></div><div className="flex flex-wrap gap-3"><button onClick={() => setPatientPickerOpen(true)} className="secondary"><Search size={17}/> Cambiar paciente</button>{canCreate && <button onClick={() => { setReport(emptyReport()); setCreatingReport(true); }} className="secondary"><FileUp size={18}/> Agregar informe</button>}{canCreate && <button onClick={() => { setRecord({ ...emptyRecord(), pacienteId: patient.codigo }); setCreating(true); }} className="primary"><ClipboardPlus size={18}/> Nueva atención</button>}</div></div>
    {loading ? <div className="rounded-2xl border bg-white p-16 text-center text-slate-400">Cargando historia clínica...</div> : <>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl border bg-white p-4"><p className="text-xs font-bold uppercase text-slate-500">Registros clínicos</p><p className="mt-2 text-2xl font-bold">{history?.records?.length || 0}</p></div><div className="rounded-2xl border bg-white p-4"><p className="text-xs font-bold uppercase text-slate-500">Informes de prácticas</p><p className="mt-2 text-2xl font-bold">{history?.reports?.length || 0}</p></div><div className="rounded-2xl border bg-white p-4"><p className="text-xs font-bold uppercase text-slate-500">Turnos</p><p className="mt-2 text-2xl font-bold">{history?.appointments?.length || 0}</p></div><div className="rounded-2xl border bg-white p-4"><p className="text-xs font-bold uppercase text-slate-500">Cabos asociados</p><p className="mt-2 text-2xl font-bold">{history?.cabos?.length || 0}</p></div></div>
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex overflow-x-auto border-b border-slate-200 bg-slate-50 p-1.5">
          {[["records", "Evoluciones y atenciones", history?.records?.length || 0], ["reports", "Informes de prácticas", history?.reports?.length || 0], ["appointments", "Turnos", history?.appointments?.length || 0], ["cabos", "Cabos", history?.cabos?.length || 0]].map(([id, label, count]) => <button key={id} type="button" onClick={() => setActiveTab(id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold ${activeTab === id ? "bg-white text-hospital-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>{label}<span className={`rounded-full px-2 py-0.5 text-xs ${activeTab === id ? "bg-hospital-100" : "bg-slate-200"}`}>{count}</span></button>)}
        </div>
      ...
      {activeTab === "reports" && <div className="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">{(history?.reports || []).map((item) => <article key={item.id} className="p-5 hover:bg-slate-50"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-700">{item.tipoPractica}</span><span className="text-sm font-semibold text-slate-500">{formatDate(item.fechaPractica)}</span>{item.registroId && <span className="text-xs text-slate-400">Atención #{item.registroId}</span>}</div><h3 className="mt-3 font-bold text-slate-800">{item.titulo}</h3>{item.descripcion && <p className="mt-2 whitespace-pre-wrap text-sm text-slate-600">{item.descripcion}</p>}<p className="mt-3 text-xs text-slate-400">Cargado por {item.registradoPor || "Usuario"}</p></div><div className="flex min-w-64 flex-col gap-2">{item.archivos.map((file) => <button key={file.id} type="button" onClick={() => openReportFile(file)} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-left text-sm hover:border-hospital-300 hover:bg-hospital-50"><span className="text-hospital-600">{String(file.tipo).startsWith("image/") ? <Image size={20}/> : <FileText size={20}/>}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-slate-700">{file.nombre}</span><span className="text-xs text-slate-400">{(Number(file.tamano) / 1024 / 1024).toFixed(2)} MB</span></span><Download size={17} className="text-slate-400"/></button>)}</div></div></article>)}{!history?.reports?.length && <div className="p-14 text-center text-slate-400"><FileText className="mx-auto mb-3" size={34}/><p>Todavía no hay informes de prácticas cargados.</p></div>}</div>}
    </>}
    ...
    {creatingReport && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4"><form onSubmit={submitReport} className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><div><p className="text-xs font-bold uppercase text-hospital-600">Historia clínica</p><h3 className="text-xl font-bold">Nuevo informe de práctica</h3></div><button type="button" onClick={() => setCreatingReport(false)} className="p-2 text-slate-400"><X/></button></div><div className="grid gap-4 p-5 sm:grid-cols-2"><Field label="Tipo de práctica" required><select value={report.tipoPractica} onChange={(e) => setReport({...report,tipoPractica:e.target.value})} className="control"><option>Laboratorio</option><option>Diagnóstico por imágenes</option><option>Cardiología</option><option>Anatomía patológica</option><option>Endoscopía</option><option>Otra práctica</option></select></Field><Field label="Fecha de realización" required><input type="date" value={report.fechaPractica} onChange={(e) => setReport({...report,fechaPractica:e.target.value})} className="control"/></Field><Field label="Título del informe" required className="sm:col-span-2"><input value={report.titulo} onChange={(e) => setReport({...report,titulo:e.target.value})} className="control" placeholder="Ej.: Hemograma completo"/></Field><Field label="Atención relacionada" className="sm:col-span-2"><div className="flex gap-2"><div className="control min-w-0 flex-1 bg-slate-50"><p className={`truncate ${selectedRelation ? "font-semibold text-slate-700" : "text-slate-400"}`}>{selectedRelation ? `${selectedRelation.type} · ${selectedRelation.date} · ${selectedRelation.title}` : "Sin atención relacionada"}</p></div><button type="button" onClick={() => setRelationPickerOpen(true)} className="secondary shrink-0"><Search size={17}/> Buscar</button></div>{selectedRelation && <button type="button" onClick={() => setReport({...report,relacion:""})} className="mt-2 text-xs font-semibold text-red-600 hover:underline">Quitar relación</button>}<p className="mt-2 text-xs text-slate-500">Buscá por fecha, profesional, Cabo, práctica, motivo o descripción.</p></Field><Field label="Descripción / resultado" className="sm:col-span-2"><textarea rows="3" value={report.descripcion} onChange={(e) => setReport({...report,descripcion:e.target.value})} className="control resize-y" placeholder="Observaciones relevantes del estudio..."/></Field><Field label="Archivos adjuntos" required className="sm:col-span-2"><input type="file" multiple accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(e) => setReport({...report,archivos:Array.from(e.target.files || [])})} className="control file:mr-3 file:rounded-lg file:border-0 file:bg-hospital-50 file:px-3 file:py-2 file:font-semibold file:text-hospital-700"/><p className="mt-2 text-xs text-slate-500">PDF, JPG, PNG o WEBP. Hasta 5 archivos de 10 MB cada uno.</p>{report.archivos.length > 0 && <div className="mt-2 space-y-1">{report.archivos.map((file) => <p key={`${file.name}-${file.size}`} className="truncate text-xs text-slate-600">• {file.name} ({(file.size / 1024 / 1024).toFixed(2)} MB)</p>)}</div>}</Field></div><div className="flex justify-end gap-3 border-t p-4"><button type="button" onClick={() => setCreatingReport(false)} className="secondary">Cancelar</button><button disabled={savingReport} className="primary disabled:opacity-50">{savingReport ? "Guardando..." : "Guardar informe"}</button></div></form></div>}
    {relationPickerOpen && <ClinicalRelationPicker options={relationOptions} selected={report.relacion} onSelect={(value) => { setReport({...report,relacion:value}); setRelationPickerOpen(false); }} onClose={() => setRelationPickerOpen(false)}/>}
